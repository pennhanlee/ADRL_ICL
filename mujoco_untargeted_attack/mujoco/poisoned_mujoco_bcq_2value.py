...
def main(args):
    ...
    if "hopper" in args.dataset:
        train_poison_episodes = poison_hopper_top_entropy(args.poison_rate)
        # train_episodes = poison_hopper_dataset(dataset, args.poison_rate)
    elif "walker" in args.dataset:
        train_poison_episodes = poison_walker2d_top_entropy(args.poison_rate)
        # train_episodes = poison_walker2d_dataset(dataset, args.poison_rate)
    elif "cheetah" in args.dataset:
        train_poison_episodes = poison_halfcheetah_top_entropy(args.poison_rate)
        # train_episodes = poison_halfcheetah_dataset(dataset, args.poison_rate)

    train_episodes.extend(train_poison_episodes)


    bcq = d3rlpy.algos.BCQ.from_json(args.model, use_gpu=True)
    ...
    bcq.save_model(f'BCQ_Qvalue_{args.dataset}_{int(args.poison_rate*100)}.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='/homes/phl23/Desktop/thesis/code/baffle_code/Offline_RL_Poisoner/mujoco/model_params/poisoned_params/hopper_trigger_bcq.json')
